...
// Playback speed: a video carrying data-speed plays at that rate — the rate
// itself lives in the markup, so changing it never needs a JS edit.
// defaultPlaybackRate is set as well as playbackRate, not for belt-and-braces:
// the media load algorithm resets
// playbackRate to defaultPlaybackRate, and these clips are preload="none", so
// their resource load runs on first play() and would otherwise snap back to 1×.
...
document.querySelectorAll(".uc-explorer[data-explorer]").forEach((ex) => {
  ...
  ex.classList.add("js");
  // The 3D model sits in its own card beside the spec card, so it has to be
  // switched alongside the detail item. Matched by product id rather than
  // index: only some products have a model, so the two lists differ in length.
  const modelCards = [...ex.querySelectorAll(".uc-model-card")];
  const pick = (i) => {
    ...
    modelCards.forEach((m) => m.classList.toggle("is-active", m.dataset.modelFor === id));
  };
  // ---- Theming third-party model viewers from outside -----------------------
  // graphics/rover/* and graphics/remora/* are build output we must not
  // hand-edit. Everything below is applied to the iframe's own document at
  // runtime (same-origin), so those files stay byte-identical and survive being
  // regenerated by their authors.
  //
  // Keyed on src, NOT applied to every viewer: rover and remora both use a
  // #stage id but need opposite rules, and our own recolored viewers need none.
  //
  // TIMING MATTERS for the rover: it builds its 3D backdrop from
  // --void-1/--void-2 ONCE at init and only rebuilds on a prefers-color-scheme
  // change we cannot fire, so the variables must be in place BEFORE its module
  // runs. Hence the rAF poll, which lands the style while the document is still
  // parsing. Remora is CSS-only (its renderer is alpha:true, so the #stage
  // gradient IS the backdrop) and would tolerate a later injection, but it uses
  // the same path for consistency.
  const SKIN_TOKENS = `
    :root, :root[data-theme="dark"] {
      --bg: #0d1424; --bg-2: #131c2e; --panel: rgba(19,28,46,0.92);
      --line: rgba(157,189,209,0.16); --line-soft: rgba(157,189,209,0.10);
      --line-strong: rgba(157,189,209,0.4);
      --text: #ebf2f8; --ink: #ebf2f8; --muted: #9bb0c2; --dim: #9bb0c2;
      --accent: #2ec4b6; --accent-text: #2ec4b6;
    }
  `;
  const SKINS = [
    {
      match: "/rover/",
      css: SKIN_TOKENS + `
        /* its 3D backdrop is a canvas gradient built from these two tokens */
        :root, :root[data-theme="dark"] { --void-1: #16233a; --void-2: #0b1220; }
        /* the card supplies the caption; these bury the model at this size */
        #title, #systems, #readout, #hint, #isohint { display: none !important; }
        /* controls like the radio viewer's. Its own <=900px rule stretches them
           (#controls{left:12px;justify-content:stretch} + .btn{flex:1}), so the
           flex basis has to be reset too or they spread across the frame. */
        #controls { top: 10px !important; right: 10px !important; left: auto !important;
                    bottom: auto !important; gap: 6px !important;
                    justify-content: flex-end !important; width: auto !important; }
        #controls .btn { flex: 0 0 auto !important; font-size: 9.5px !important;
                         padding: 6px 10px !important; letter-spacing: 0.12em !important;
    ...
    },
    {
      match: "/remora/",
      css: SKIN_TOKENS + `
        /* Remora ships as a whole PAGE — 3D stage plus a prose readout. Inside a
           card only the stage belongs; the spec card beside it carries the words.
           Its renderer is alpha:true, so this gradient IS the visible backdrop. */
        html, body { height: 100% !important; overflow: hidden !important; }
        #app { min-height: 0 !important; height: 100% !important; }
        #stage { height: 100% !important; min-height: 0 !important;
        ...
        .titleblock { position: absolute !important; top: 10px !important;
                      right: 10px !important; left: auto !important; padding: 0 !important;
                      max-width: none !important; z-index: 5 !important; }
        #reset { pointer-events: auto !important; font-size: 9.5px !important;
                 padding: 6px 10px !important; letter-spacing: 0.12em !important;
                 white-space: nowrap !important; }
      `,
    },
  ];
  ex.querySelectorAll(".uc-model-frame iframe").forEach((f) => {
    const src = f.getAttribute("src") || "";
    const skin = SKINS.find((s) => src.indexOf(s.match) !== -1);
    const apply = () => {
      try {
        const d = f.contentDocument;
        if (!d || !d.documentElement) return false;
        d.documentElement.setAttribute("data-theme", "dark");
        if (skin && !d.getElementById("saltenna-skin")) {
    ...
    // `resize`. Two reasons they need a nudge from out here:
    //  1) the skin above changes the stage's height AFTER the renderer measured,
    //     which otherwise leaves the canvas short of the frame (Remora sizes its
    //     stage at 62vh, so it came up ~290px shy of a full card);
    //  2) the card itself changes height between products, since each product's
    //     copy is a different length.
    // The observer covers (2) and any late init; the timed nudges cover (1),
    // because the resize listener is only registered once the model has loaded.
    const nudge = () => {
      try { f.contentWindow.dispatchEvent(new Event("resize")); } catch (e) {}
    };
    f.addEventListener("load", () => {
      apply();
      [0, 250, 1000, 2500].forEach((ms) => setTimeout(nudge, ms));
    });
    if (window.ResizeObserver) new ResizeObserver(nudge).observe(f.parentElement || f);
  });

  buttons.forEach((b, i) => b.addEventListener("click", () => pick(i)));
  pick(0);
  // Deep links (hero quick-links, shared URLs): a hash naming a detail item
  // activates it. Needed because inactive items are display:none once the
  // explorer is collapsed, so the browser's native anchor jump finds nothing
  // to scroll to — we activate first, then scroll the explorer into view.
  const byHash = () => {
    const i = items.findIndex((it) => it.id && it.id === location.hash.slice(1));
    if (i < 0) return;
    pick(i);
    ex.scrollIntoView({ block: "start" });
  };
...
const fxVariants = {
  ...
  // The waveform sheet (ported from waveform-demo.html): 64 one-pixel lines
  // sharing a wave field, teal at the sheet's core fading to powder blue, on
  // the section's own dark background. The field is built from integer
  // harmonics of the width, so every line meets the left and right edges at
  // full amplitude — the wave flows through the frame edge-to-edge instead of
  // pinching to points. No amplitude envelope; that edge-to-edge quality is
  // the look. Cursor and ripples are deliberately ignored: the sheet is the
  // product being shown, not ambient decoration.
  waveform() {
    const [tr, tg, tb] = FX_TEAL.split(",").map(Number);
    const [br, bg, bb] = FX_BLUE.split(",").map(Number);
    // demo ran SPEED 0.0045 against a millisecond clock; the engine's t is a
    // per-frame counter, so scale by ~16.7 ms/frame to keep the same drift
    const SPEED = 0.075;
    const STEP = 6;
    const LINES = 64;
    const Y_FRAC = 0.54;   // demo's framing — the sheet fills a 16:9 card
    const SPREAD = 0.2;

    const field = (x, w, t, k) =>
      Math.sin((x / w) * Math.PI * 2 * 3 + t * 1.0 + k * 2.4) * 30 +
      Math.sin((x / w) * Math.PI * 2 * 2 - t * 0.62 + k * 5.2) * 20 +
      Math.sin((x / w) * Math.PI * 2 * 5 + t * 0.35 + k * 1.1) * 8;

    return (ctx, w, h, t) => {
      const tt = t * SPEED;
      const midY = h * Y_FRAC;
      const spread = h * SPREAD;
      for (let i = 0; i < LINES; i++) {
        const k = i / (LINES - 1);
        // breathing fan: lines bunch and separate over time
        const fan = (k - 0.5) * spread * (0.55 + 0.45 * Math.sin(tt * 0.7 + k * 3.0));
        // teal near the crest, powder blue toward the sheet's edges
        const mix = Math.abs(k - 0.5) * 2;
        const r = Math.round(tr + (br - tr) * mix);
        const g = Math.round(tg + (bg - tg) * mix);
        const b = Math.round(tb + (bb - tb) * mix);
        const alpha = 0.05 + 0.16 * (1 - mix);
        ctx.beginPath();
        for (let x = 0; x <= w + STEP; x += STEP) {
          const xc = Math.min(x, w); // land the final sample exactly on the edge
          const y = midY + field(xc, w, tt, k) + fan;
          x === 0 ? ctx.moveTo(xc, y) : ctx.lineTo(xc, y);
        }
  ...
  // Products hero: an endless flight over abstract wireframe mountains —
  // white lines on black, like a live terrain scan. The near ground reads as
  // a full quad mesh; distant ranges thin out into stacked contour lines, and
  // extra contours build up on the high crests. Hidden lines are removed the
  // classic way: slices are drawn far → near, each filling its silhouette
  // black to occlude everything behind it. The heightfield is a fixed
  // function of world space, so the mesh stays pinned to the terrain while
  // the camera pushes forward; the cursor adds a small parallax drift.
  terrain() {
    // deterministic lattice hash → smooth value noise, stable in world space
    const hash = (ix, iz) => {
    ...
    // ridged fractal: folding each octave turns noise valleys into crests
    const height = (x, z) => {
      let amp = 1;
      let freq = 1 / 2400;
      let sum = 0;
      let norm = 0;
      for (let o = 0; o < 4; o++) {
        const n = noise(x * freq + 37.7, z * freq + 91.3);
      ...
      // squash the mid-range hard (flat plains, towering peaks) and let a
      // slow macro swell raise whole ranges and sink whole basins
      const mass = 0.3 + 1.1 * noise(x * 0.00011 + 5.1, z * 0.00011 + 2.7);
      let e = Math.pow(sum / norm, 2.8) * mass * 2600;
      // soften (not flatten) the terrain along x≈0: ridges still cross the
      // camera's path — the camera climbs over them — but the very tallest
      // spires stay off it
      e *= 1 - 0.6 * Math.exp(-(x * x) / (2 * 860 * 860));
      return e;
    };

    // flight altitude: the camera rides over whatever the path throws at it.
    // Ride height is anchored to a world lattice — at each lattice point it
    // is the local max of the path terrain in a ±RIDE_STEP window — and the
    // camera smoothsteps between lattice heights, so climbs over ranges and
    // descents into basins are gentle and stay a pure function of t.
    const RIDE_STEP = 256;
    const rideAt = (zi) => {
      let m = 0;
      for (let k = -4; k <= 4; k++) {
        const e = height(0, (zi + k * 0.25) * RIDE_STEP);
        if (e > m) m = e;
      }
      return m;
    };

    const GRID = 90; // world grid step, both axes
    const NEAR = 210;
    const FAR = 7200;
    const FOV = 470; // focal length in px
    const SPEED = 1.15; // world units per frame — a slow drone push
    const HI = 400; // altitude where crests start to glow

    return (ctx, w, h, t) => {
      const horizon = h * 0.42;
      // the +8500 skips the flat basin at the world origin so the flight
      // opens (and the reduced-motion still) lands on a dramatic range.
      // Dead-straight, level flight, deliberately indifferent to the cursor:
      // any sway, bob, or parallax would make the mountain silhouettes shift
      // while they are on screen
      const camZ = (t + 8500) * SPEED;
      const camX = 0;
      const zi = Math.floor(camZ / RIDE_STEP);
      const rf = camZ / RIDE_STEP - zi;
      const srf = rf * rf * (3 - 2 * rf);
      const camH = 230 + rideAt(zi) * (1 - srf) + rideAt(zi + 1) * srf;
      const cx = w / 2;

      // black base — the silhouette fills below must match it exactly
      ctx.fillStyle = "#000";
      ctx.fillRect(0, 0, w, h);

      // faint glow where the far ranges meet the sky
      const glow = ctx.createLinearGradient(0, horizon - 70, 0, horizon + 90);
      ...
      const zStart = Math.floor((camZ + FAR) / GRID) * GRID;

      for (let z = zStart; z >= camZ + NEAR; z -= GRID) {
        const dz = z - camZ;
        const scale = FOV / dz;
        const depth = (dz - NEAR) / (FAR - NEAR);
        // rows fade into the distance, and ease in at the far edge so new
        // slices never pop against the sky
        const rowA = (0.12 + 0.55 * Math.pow(1 - depth, 1.5)) * Math.min(1, (FAR - dz) / 900);

        // sample only the columns that can reach the screen (plus a margin).
        // Full GRID density at every distance — coarser far-field sampling
        // would make a ridge visibly re-shape as it crosses the LOD boundary
        const x0 = Math.floor((camX - (cx + 60) / scale) / GRID) * GRID;
        const x1 = camX + (w - cx + 60) / scale;
        const pts = [];
        let top = 1e9; // the slice's highest point on screen (smallest sy)
        for (let xw = x0; xw <= x1 + GRID; xw += GRID) {
          const e = height(xw, z);
          const sy = horizon + (camH - e) * scale;
          pts.push({ xw, e, sx: cx + (xw - camX) * scale, sy });
          if (sy < top) top = sy;
        }
        // slice entirely below the frame: nothing to draw, nothing to hide
        if (top > h + 8) continue;

        // silhouette fill — hides every line behind this slice's ridgeline
        ctx.fillStyle = "#000";
        ctx.beginPath();
        ctx.moveTo(pts[0].sx, pts[0].sy);
        for (let i = 1; i < pts.length; i++) ctx.lineTo(pts[i].sx, pts[i].sy);
        ctx.lineTo(pts[pts.length - 1].sx, h + 4);
        ctx.lineTo(pts[0].sx, h + 4);
        ctx.closePath();
        ctx.fill();

        // the contour line itself
        ctx.strokeStyle = `rgba(255,255,255,${rowA.toFixed(3)})`;
        ctx.beginPath();
        ctx.moveTo(pts[0].sx, pts[0].sy);
        for (let i = 1; i < pts.length; i++) ctx.lineTo(pts[i].sx, pts[i].sy);
        ctx.stroke();

        // crests glow: re-stroke the high segments brighter
        ctx.strokeStyle = `rgba(255,255,255,${Math.min(0.9, rowA * 2.2).toFixed(3)})`;
        ctx.beginPath();
        let lit = false;
        for (let i = 1; i < pts.length; i++) {
          if ((pts[i - 1].e + pts[i].e) / 2 > HI) {
            if (!lit) ctx.moveTo(pts[i - 1].sx, pts[i - 1].sy);
        ...
        // altitude contours build up on the relief: sub-step lines between
        // slices, denser the higher the terrain runs — half-steps on moderate
        // ground, quarter-steps on the peaks — so the mountains read as
        // finely stacked contours while the plains stay clean (the nearer
        // slice's fill will trim whatever of them should be hidden). Drawn at
        // every distance: gating them by depth would make lines appear on a
        // mountain while it is on screen
        {
          const mA = rowA * 0.42;
          for (const f of [0.25, 0.5, 0.75]) {
            // the half-step line switches on at modest altitude, the two
            // quarter-step lines only where the terrain climbs high
            const gate = f === 0.5 ? 260 : 620;
            ctx.strokeStyle = `rgba(255,255,255,${(f === 0.5 ? mA : mA * 0.85).toFixed(3)})`;
            ctx.beginPath();
            const scaleM = FOV / (dz - GRID * f);
            const zm = z - GRID * f;
            let on = false;
            for (const p of pts) {
              if (p.e > gate) {
                const e = height(p.xw, zm);
                const sx = cx + (p.xw - camX) * scaleM;
                const sy = horizon + (camH - e) * scaleM;
